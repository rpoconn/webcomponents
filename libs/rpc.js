...
rpc.ServiceProxy.prototype.__callMethod = function(methodName, params, successHandler, exceptionHandler, completeHandler){
	...
	try {
		...
		if(this.__protocol == 'JSON-RPC'){ //then this.__isAsynchronous is implied
		...
		//Calls made with XMLHttpRequest ------------------------------------------------------------
			...
			if(this.__protocol == 'XML-RPC'){
                ...
                if (window.userName) {
                    var encodedData = ""
                    if (window.userName && window.password !== null) {
                        encodedData = base64.encode(window.userName + ":" + window.password);
                    }
...
rpc.ServiceProxy.prototype.__doCallback = function(response){
	...
	if(rpc.pendingRequests[response.id] && rpc.pendingRequests[response.id].scriptElement){
...
rpc.ServiceProxy.prototype.__toXMLRPC = function(value){
	...
	switch(typeof value){
		...
		case 'string':
			xml.push('<string>');
			xml.push(value.encodeHTML()); //escape for XML!
			xml.push('</string>');
			break;
		case 'object':
			if(value === null)
